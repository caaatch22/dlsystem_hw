...
// strides
__device__ size_t index_transform(size_t index, CudaVec shape, CudaVec strides,
                                  size_t offset) {
  ...
  for (int i = 0; i < shape.size; i++) comp_idx += idxs[i] * strides.data[i];
  return comp_idx;
}
...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size,
  ...
  /// BEGIN SOLUTION
  if (gid < size) out[gid] = a[index_transform(gid, shape, strides, offset)];
  /// END SOLUTION
}
...
__global__ void EwiseSetitemKernel(const scalar_t* a, scalar_t* out,
                                   ...
                                   size_t offset) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size) out[index_transform(gid, shape, strides, offset)] = a[gid];
}
...
__global__ void ScalarSetitemKernel(size_t size, scalar_t val, scalar_t* out,
                                    ...
                                    size_t offset) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size) out[index_transform(gid, shape, strides, offset)] = val;
}
...
__global__ void ReduceMaxKernel(const scalar_t* a, scalar_t* out, size_t size,
  ...
  if (gid < size) {
    ...
    scalar_t reduce_max = a[start];
    for (size_t i = 1; i < reduce_size; i++) {
      reduce_max = max(reduce_max, a[start + i]);
    }
    out[gid] = reduce_max;
  }
}
